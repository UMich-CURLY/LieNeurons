...
class EulerPoincareEquation(nn.Module):
    ...
    def forward(self,t,w):
        ...
        '''
        w_v = w.unsqueeze(2)
        # print("t: ",t,"u: ",self.ufunc(t))
        # print("I_inv*u:", torch.matmul(self.I_inv,self.ufunc(t)))
        # print("value before: ",-torch.matmul(self.I_inv,torch.matmul(self.hat_layer(w),torch.matmul(self.I,w_v))).squeeze(2))
        # print("value: ",-torch.matmul(self.I_inv,torch.matmul(self.hat_layer(w),torch.matmul(self.I,w_v))).squeeze(2)\
        #         +torch.matmul(self.I_inv,self.ufunc(t)) )
...
def visualize(true_y, pred_y, odefunc, itr):
    ...
    plt.xlabel('t')
    plt.ylabel('x, y, z')
    plt.legend(['Ground Truth x', 'Ground Truth y', 'Ground Truth z', 'Lie Neurons x', 'Lie Neurons y', 'Lie Neurons z'])
    ...
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    ax.legend(['Ground Truth', 'Lie Neurons'])
    # fig = plt.figure(figsize=(12, 4), facecolor='white')
    # ax_traj = fig.add_subplot(131, frameon=False)
    # ax_phase = fig.add_subplot(132, frameon=False)
    # ax_vecfield = fig.add_subplot(133, frameon=False)
    ...
    dydt = odefunc(torch.tensor(0), rearrange(torch.Tensor(np.stack([x, y, np.zeros_like(y)], -1).reshape(21 * 21, 3)),'b d -> b 1 d').to(device)).cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    elif args.model_type == 'LN_ode9':
        func = LNODEFunc9(device=device).to(device)
    elif args.model_type == 'LN_ode10':
        func = LNODEFunc10(device=device).to(device)
    elif args.model_type == 'LN_ode11':
