...
if args.model_save_path == 'unspecified':
    args.model_save_path = "weights/Euler_WithInput/Multi_Tra/"+args.model_type + "/"
...
if args.log_writer_path == 'unspecified':
    args.log_writer_path = "logs/Euler_WithInput/Multi_Tra/"+args.model_type + "/"
...
t = data_train['t'] # tensor, tensor_size =  [num_sample]
training_y0 = data_train['y0'] #tensor, tensor_size =  [num_traj, 3]
...
print("number of training trajectories: ", args.num_training)
if args.num_training > len(training_y):
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                u_temp = torch.stack([training_u[args.num_training + i] for i in range(num_val)], dim=1) # u_temp: time_series * num_val * 1 * 3
                val_y = torch.stack([training_y[args.num_training + i] for i in range(num_val)], dim=1) # val_y: time_series * num_val * 1 * 3
                y0_temp = training_y0[args.num_training:args.num_training + num_val,:] # y0_temp: num_val * 3
                y0_temp = y0_temp.unsqueeze(1) # y0_temp: num_val * 1 * 3

                func_with_u_val = lambda t,y: func(t,y,u_temp,Dt)
                pred_y = odeint(func_with_u_val, y0_temp, t) # pred_y: time_series * 2 * 1 * 3

                loss = torch.mean(torch.abs(pred_y - val_y))

                writer.add_scalar('validation loss', loss.item(), itr)
                
                print('Iter {:04d} | Total Loss {:.6f}'.format(itr, loss.item()))
                if args.viz:
                    Visualizer_train.Plot_XYZ_Tra(training_y[args.num_training],pred_y[:,0,:,:],t,iter=itr)
